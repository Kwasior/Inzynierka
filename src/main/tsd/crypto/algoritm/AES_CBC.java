...
public class AES_CBC {
    ...
    public static byte[] encryptBloc(byte[] in) {
        ...
        for (int round = 1; round < Nr; round++) {
            ...
            //	System.out.println( round + "Rotacja");
            for (int i = 0; i < tmp.length; i++) {
    ...
    public static byte[] encrypt(byte[] in, byte[] key, byte[] iv) {
    ...
    public static byte[] decrypt(byte[] in, byte[] key, byte[] iv, int length) {
        int i;
        byte[] tmp = new byte[in.length];
        byte[] bloc = new byte[16];
        byte[] bloc2 = new byte[16];
        byte[] iv2 = new byte[in.length];
        ...
        for (int j = length; j < iv.length; j++) {
            tmp[j] = 0x00;
        }
